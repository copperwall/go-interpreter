...
func testExpectedObject(t *testing.T, expected any, actual object.Object) {
	...
	case []int:
		...
		if !ok {
			t.Errorf("Expected value to be object.Array, got %T (%+v)", actual, actual)
		}

		if len(expected) != len(array.Elements) {
			t.Errorf("Expected %d elements, got %d", len(expected), len(array.Elements))
		}
	...
	case map[object.HashKey]int64:
		...
		if !ok {
			t.Errorf("Expected Hash, got %T (%+v)", actual, actual)
		}

		if len(hash.Pairs) != len(expected) {
			t.Errorf("Wrong map length detected, expected %d pairs, got %d", len(expected), len(hash.Pairs))
		}
...
func TestIndexExpresions(t *testing.T) {
	tests := []vmTestCase{
		{"[1,2,3][1]", 2},
		{"[1,2,3][0 + 2]", 3},
		{"[[3,1,1]][0][0]", 3},
...
func TestCallingFunctionsWithReturnStatement(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `
				let earlyExit = fn() { return 10; return 5; }
				earlyExit();
			`,
			expected: 10,
		},
