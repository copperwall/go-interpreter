...
func TestFunctionCalls(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input: `
				let manyArg = fn(a,b,c) { let d = a; a; b; c; d; };
				manyArg(42,43,44);
			`,
			expectedConstants: []any{
				[]code.Instructions{
...
func TestCompilerScopes(t *testing.T) {
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 1 {
		t.Errorf("Number of instructions in scope wrong. got=%d, want=%d", len(compiler.scopes[compiler.scopeIndex].instructions), 1)
	}
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 2 {
		t.Errorf("Number of instructions in scope wrong. got=%d, want=%d", len(compiler.scopes[compiler.scopeIndex].instructions), 2)
	}
...
func testStringObject(expected string, actual object.Object) error {
	...
	if !ok {
		return fmt.Errorf("Expected object.String, got %T (%+v)", actual, actual)
	}

	if result.Value != expected {
		return fmt.Errorf("Expected string to be %q, got %q instead", expected, result.Value)
	}
	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []any{
				...
				[]code.Instructions{
					code.Make(code.OpGetLocal, 0), //  Will be explained later.
